...
export class CrawlConferenceProcessor extends WorkerHost {
    ...
    async process(job: Job, token?: string): Promise<any> {
        switch(job.name){
            case ProcessCrawlToken.CRAWL_NEW_CONF_JOB_NAME:
                const conference = await this.crawlConference(job.data);
                const conferenceId = await this.cacheManager.get(`job-${job.id}`) as string;
                if (!conference) {
    ...
    async crawlConference(input : ConferenceCrawlInput) :Promise<ConferenceCrawlData|null> {
    ...
    public parseDates(dateStr : string) : KeyDates[] {
        ...
        const parsedDates = dateStr.split('\n').reduce((acc : KeyDates[] , line) => {
            ...
            if (key && value) {
                ...
                let date_value = new Date(parser.fromString(value.trim()).toUTCString());
